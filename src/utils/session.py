...
def login_user(email: str, password: str, remember_me: bool = False):
    ...
    if user and bcrypt.checkpw(password.encode("utf-8"), user.password_hash.encode("utf-8")):
        ...
        if remember_me:
            ...
            # store token in DB (simple approach: reuse subscription field or add a new column)
            user.subscription = f"{user.subscription}|token:{token}"
            db.commit()
        log_event(email, "login")
        db.close()
        return True
...
def restore_user():
    ...
    if not token:
        return None
    db = SessionLocal()
    user = db.query(User).filter(User.subscription.like(f"%token:{token}%")).first()
    db.close()
    if user:
        st.session_state["user"] = {
            "id": user.id,
            "email": user.email,
            "role": user.role,
            "subscription": user.subscription.split("|")[0]  # strip token
        }
